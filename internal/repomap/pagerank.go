...
func getVector(n int) []float64 {
	if v, ok := vectorPool.Get().([]float64); ok && cap(v) >= n {
		return v[:n]
	}
	return make([]float64, n)
}

// putVector zeroes and returns v to the pool.
func putVector(v []float64) {
	for i := range v {
		v[i] = 0
	}
	vectorPool.Put(v[:cap(v)])
}
...
func runPageRank(graph *FileGraph, personalization map[string]float64) (map[string]float64, []float64, bool) {
	...
	for range pageRankIterations {
		...
		// Return previous rank vector to pool, keep next as current.
		putVector(rank)
		rank = next
