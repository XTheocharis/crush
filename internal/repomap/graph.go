...
// ImportEdge captures a local import relationship from one file to an
// imported package path. The Category field is one of "stdlib",
// "third_party", "local", "unknown" — only "local" edges are wired into
// the graph.
...
// BuildGraphOptions configures graph construction.
...
func buildGraph(tags []treesitter.Tag, chatFiles []string, mentionedIdents []string, opts ...BuildGraphOptions) *FileGraph {
	...
	if len(imports) > 0 {
		...
		for _, node := range nodeList {
			...
			dirToFiles[dir] = append(dirToFiles[dir], node)
		}
		...
		for _, imp := range imports {
			...
			from := normalizeGraphRelPath(imp.From)
			if from == "" {
				continue
			}
			resolved := resolveImportToFiles(imp.ImportPath, dirToFiles)
...
func resolveImportToFiles(importPath string, dirToFiles map[string][]string) []string {
	clean := strings.TrimSuffix(importPath, "/")
	if clean == "" {
		return nil
	}
	if files, ok := dirToFiles[clean]; ok {
		return files
	}
	for dir, files := range dirToFiles {
		if dir != "" && (strings.HasSuffix(clean, "/"+dir) || clean == dir) {
			return files
		}
	}
	return nil
