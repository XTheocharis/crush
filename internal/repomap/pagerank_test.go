...
// buildBenchGraph creates a synthetic file graph with n nodes and a
// realistic edge density (~3 outgoing edges per node with varying weights).
func buildBenchGraph(n int) *FileGraph {
	nodes := make([]string, n)
	for i := range n {
		nodes[i] = fmt.Sprintf("pkg/node%03d.go", i)
	}

	var edges []GraphEdge
	for i := range n {
		// Each node links to 3 successors (modular wrap).
		for j := 1; j <= 3; j++ {
			to := (i + j) % n
			edges = append(edges, GraphEdge{
				From:   nodes[i],
				To:     nodes[to],
				Ident:  fmt.Sprintf("Func%03d", to),
				Weight: float64(j),
			})
		}
	}
...
func BenchmarkPageRank(b *testing.B) {
	graph := buildBenchGraph(64)
	pers := map[string]float64{"pkg/node000.go": 1.0}
	for b.Loop() {
		Rank(graph, pers)
	}
}
