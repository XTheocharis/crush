...
func (s *Service) parseFile(ctx context.Context, parser treesitter.Parser, rootDir, relPath string, forceRefresh bool, cache map[string]fileCacheEntry) fileParseResult {
	...
	if analysis != nil {
		...
		for _, imp := range analysis.Imports {
			imports = append(imports, treesitter.ImportInfo{
				Path:     imp.Path,
				Names:    imp.Names,
...
func (s *Service) writePathImports(ctx context.Context, tx *sql.Tx, repoKey string, r fileParseResult) error {
	if _, err := tx.ExecContext(ctx,
		"DELETE FROM repo_map_imports WHERE path = ? AND repo_key = ?",
		r.relPath, repoKey,
	); err != nil {
		return fmt.Errorf("delete existing imports for %q: %w", r.relPath, err)
	}
	for _, imp := range r.imports {
		if _, err := tx.ExecContext(ctx,
			"INSERT INTO repo_map_imports (path, import_path, category, repo_key) VALUES (?, ?, ?, ?)",
			r.relPath, imp.Path, imp.Category, repoKey,
		); err != nil {
			return fmt.Errorf("insert import for %q: %w", r.relPath, err)
		}
	}
	return nil
}
...
func (s *Service) extractTags(ctx context.Context, rootDir string, fileUniverse []string, forceRefresh bool) ([]treesitter.Tag, []ImportEdge, error) {
	if s == nil {
	...
	if s.db == nil || s.rawDB == nil {
		return nil, nil, fmt.Errorf("repo-map database is not configured")
	}
	...
	if repoKey == "" {
		return nil, nil, fmt.Errorf("repo key is empty")
	}

	parser := s.ensureParser()
	if parser == nil {
		return nil, nil, fmt.Errorf("tree-sitter parser is not available")
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("begin repo-map tag transaction: %w", err)
	}
	committed := false
	defer func() {
		if committed {
	...
	for _, r := range results {
		...
		if err := s.writePathImports(ctx, tx, repoKey, r); err != nil {
			slog.Warn("Failed to write imports for path",
				"path", r.relPath,
				"error", err)
			continue
		}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("list repo-map tags: %w", err)
	}

	importRows, err := tx.QueryContext(ctx,
		"SELECT path, import_path, category FROM repo_map_imports WHERE repo_key = ?",
		repoKey,
	)
	if err != nil {
		return nil, nil, fmt.Errorf("list repo-map imports: %w", err)
	}
	defer importRows.Close()
	var importEdges []ImportEdge
	for importRows.Next() {
		var ie ImportEdge
		if err := importRows.Scan(&ie.From, &ie.ImportPath, &ie.Category); err != nil {
			return nil, nil, fmt.Errorf("scan repo-map import: %w", err)
		}
		importEdges = append(importEdges, ie)
	}
	if err := importRows.Err(); err != nil {
		return nil, nil, fmt.Errorf("iterate repo-map imports: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return nil, nil, fmt.Errorf("commit repo-map tag transaction: %w", err)
	}
	committed = true
