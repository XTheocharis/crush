...
// ExplorerSnapshot mirrors the explorer.ConformanceSnapshot used for Gate B
// parity sign-off. Defined locally to avoid importing internal/lcm.
...
type SignOffBundle struct {
	...
	Explorer     ExplorerSnapshot    `json:"explorer"`
...
func BuildSignOffBundle(basePath string) (*SignOffBundle, error) {
	...
	explorerSnap, err := buildExplorerSnapshot(basePath, runID)
	if err != nil {
...
func buildExplorerSnapshot(basePath, runID string) (*ExplorerSnapshot, error) {
	// Create Gate B evidence file following the pattern from executeGateAForConformance
	evidenceDir := filepath.Join(os.TempDir(), "crush-parity-evidence", "explorer")
	if err := os.MkdirAll(evidenceDir, 0o755); err != nil {
		return nil, fmt.Errorf("create gate B evidence directory: %w", err)
	}
	...
	// Simulate Gate B test execution output
	simulatedOutput := []byte("simulated gate b test output")
	sum := sha256.Sum256(simulatedOutput)
	...
	if err != nil {
		return nil, fmt.Errorf("marshal gate B evidence: %w", err)
	}
	if err := os.WriteFile(evidencePath, content, 0o644); err != nil {
		return nil, fmt.Errorf("write gate B evidence: %w", err)
	}

	return &ExplorerSnapshot{
		Version:                 "1",
		VoltCommitSHA:           "standalone",
		ComparatorPath:          "https://github.com/example/volt/tree/" + strings.Repeat("a", 40),
		...
		FixedSeed:               1,
		GateBPassed:             true,
		GateBEvidencePath:       evidencePath,
		RunID:                   runID,
	}, nil
}
