...
func TestTagsExtractPersistsAndDeterministicOrder(t *testing.T) {
	...
	tags, _, err := svc.extractTags(ctx, root, []string{"src/b.go", "./src/a.go"}, false)
	require.NoError(t, err)
...
func TestTagsExtractPrunesDeletedPath(t *testing.T) {
	...
	_, _, err = svc.extractTags(ctx, root, []string{"alive.go", "gone.go"}, false)
	require.NoError(t, err)

	require.NoError(t, os.Remove(filepath.Join(root, "gone.go")))
	_, _, err = svc.extractTags(ctx, root, []string{"alive.go"}, false)
	require.NoError(t, err)
...
func TestTagsExtractUpdatesChangedFile(t *testing.T) {
	...
	_, _, err = svc.extractTags(ctx, root, []string{"x.go"}, false)
	require.NoError(t, err)
