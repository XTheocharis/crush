...
// LoadImportQuery returns embedded import query content for a language key.
func LoadImportQuery(lang string) ([]byte, error) {
	name := strings.TrimSpace(lang)
	if name == "" {
		return nil, fmt.Errorf("language key is empty")
	}
	return queriesFS.ReadFile("queries/" + name + "-imports.scm")
}
