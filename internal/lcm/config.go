...
// PerStepInjectionOverhead is a worst-case fixed estimate for per-step
// synthetic messages injected into prepared messages but not tracked by LCM:
// repo map wrapper text (~86 tokens) + todo reminder (~90 tokens, injected
// only when todo list is empty) + per-message framing (~24 tokens: 3
// messages × ~8 tokens each for role/content API encoding overhead).
const PerStepInjectionOverhead = 200

// ComputeBudget computes the soft and hard token thresholds.
//
// Formula:
//
//	overhead = systemPromptTokens + toolTokens + repoMapTokens + PerStepInjectionOverhead
...
func ComputeBudget(cfg BudgetConfig) Budget {
	overhead := cfg.SystemPromptTokens + cfg.ToolTokens + cfg.RepoMapTokens + PerStepInjectionOverhead
