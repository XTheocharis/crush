...
func newTreeSitterExplorer(parser any, profile OutputProfile) Explorer {
	p, ok := parser.(treesitter.Parser)
	if !ok {
		return nil
	}
	return &TreeSitterExplorer{parser: p, formatterProfile: profile}
}
