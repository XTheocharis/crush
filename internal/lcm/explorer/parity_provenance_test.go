...
func TestParityProvenanceBundleValidateRequiredFields(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Error(t, err)
			require.Contains(t, err.Error(), tt.want, "expected error to contain %q, got %q", tt.want, err.Error())
		})
	}
}
...
func TestParityProvenanceBundleValidateComparatorTupleOptional(t *testing.T) {
	...
	require.NoError(t, bundle.Validate(false))
}
...
func TestParityProvenanceBundleValidateInvalidDigests(t *testing.T) {
	...
	bundle := validExplorerBundle()
	bundle.VoltCommitSHA = "short"
	require.Error(t, bundle.Validate(false), "expected invalid volt_commit_sha error")
...
func TestRunParityHarnessPreflight(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "grep_ast_provenance", "expected grep_ast_provenance error, got %q", err.Error())
}
...
func TestProtocolArtifact_TokenizerSupportInvalidVersion(t *testing.T) {
	...
	require.Contains(t, err.Error(), "unsupported version", "expected version error, got: %v", err)
}
...
func TestProtocolArtifact_TokenizerSupportMissingFields(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Error(t, err)
			require.Contains(t, err.Error(), tt.want, "expected error to contain %q, got: %v", tt.want, err)
		})
	}
}
...
func TestProtocolArtifact_ExplorerFamilyMatrixMethods(t *testing.T) {
	...
	exp := efm.GetExplorerByID("JSONExplorer")
	require.NotNil(t, exp, "should find JSONExplorer")
	require.Equal(t, "JSONExplorer", exp.ExplorerID)

	nativeExplorers := efm.GetExplorersByType("data_format_native")
	require.Greater(t, len(nativeExplorers), 0, "should have data format explorers")

	found := false
	for _, e := range nativeExplorers {
		if e.ExplorerID == "JSONExplorer" {
			found = true
			break
		}
	}
