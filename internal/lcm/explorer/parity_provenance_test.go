...
func TestParityProvenanceBundleValidateInvalidDigests(t *testing.T) {
	...
	if err := bundle.Validate(false); err == nil {
		t.Fatalf("expected invalid volt_commit_sha error")
	}
...
func TestRunParityHarnessPreflight(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "grep_ast_provenance") {
		t.Fatalf("expected grep_ast_provenance error, got %q", err.Error())
	}
}
