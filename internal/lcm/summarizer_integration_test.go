...
func TestIntegration_Summarizer_Summarize(t *testing.T) {
	...
	// The five-level escalation guarantees the output is smaller than the
	// input (normal → extractive → aggressive → skeleton → deterministic
	// truncation). Verify that the full pipeline actually achieves compression.
	require.Less(t, tokens, inputTokens,
		"summary tokens (%d) should be less than input tokens (%d) after five-level escalation",
		tokens, inputTokens)
