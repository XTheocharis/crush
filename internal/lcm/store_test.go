...
func TestKindConstants(t *testing.T) {
	...
	require.Equal(t, "repo", KindRepo)
	require.Equal(t, "archive_stub", KindArchiveStub)

	// Verify all kinds are distinct.
	kinds := []string{KindLeaf, KindCondensed, KindRepo, KindArchiveStub}
...
func TestCreateArchiveStub(t *testing.T) {
	t.Parallel()
	queries, sqlDB := setupTestDB(t)
	store := newStore(queries, sqlDB)
	ctx := context.Background()

	sessionID := "sess-archive-stub"
	createTestSession(t, queries, sessionID)

	err := store.CreateArchiveStub(ctx, "sum_original123456", sessionID, "some original content that was summarized", 1000)
	require.NoError(t, err)

	// Verify the stub appears in the summaries for the session.
	summaries, err := queries.ListLcmSummariesBySession(ctx, sessionID)
	require.NoError(t, err)
	require.Len(t, summaries, 1)
	require.Equal(t, KindArchiveStub, summaries[0].Kind)
	require.Contains(t, summaries[0].Content, "[Archived from sum_original123456]")
	require.Contains(t, summaries[0].Content, "some original content")
