...
// AgentConfigRestorer is responsible for restoring checkpointed session agent
// configuration (skills, tools, agents) after compaction. Defined here to
// avoid a circular import on the agent package.
//
...
type AgentConfigRestorer interface {
	// RestoreAgentConfig restores the checkpointed agent configuration for
	// the given session. The payload contains the serialized skill/tool state
	// that was saved before compaction.
...
// Compact executes the 5-step restore sequence and clears the preserved
...
func (p *PostCompactCleaner) Compact(ctx context.Context, _ Budget) (*CompactionLayerResult, error) {
	...
	// Step 5: Restore agent configuration (skills, tools, agents).
	if restored, err := p.restoreAgentConfig(ctx, pc); err != nil {
		if firstErr == nil {
			firstErr = fmt.Errorf("restoring agent config: %w", err)
		}
	} else if restored {
		itemsRestored++
	}
...
// restoreAgentConfig restores checkpointed agent configuration (skills, tools,
// agents) from preserved data by delegating to the configured
// AgentConfigRestorer. If no restorer is configured, this is a graceful no-op.
func (p *PostCompactCleaner) restoreAgentConfig(ctx context.Context, pc *PreservedContext) (bool, error) {
	if pc.SkillState == "" {
		return false, nil
	}
	payload, err := parseSkillState(pc.SkillState)
	if err != nil {
		return false, fmt.Errorf("parsing skill state: %w", err)
	}
	if payload == nil {
		return false, nil
	}
	if p.cfg.AgentConfigRestorer != nil {
		if err := p.cfg.AgentConfigRestorer.RestoreAgentConfig(ctx, payload); err != nil {
			return false, fmt.Errorf("restoring agent config: %w", err)
		}
	}
	return true, nil
}

// skillStatePayload is the JSON structure stored in PreservedContext.SkillState.
type skillStatePayload struct {
	// Skills lists the names of skills that were loaded at compaction time.
	Skills []string `json:"skills,omitempty"`

	// ToolsDelta lists the names of MCP tools that were loaded at compaction
	// time.
	ToolsDelta []string `json:"tools_delta,omitempty"`

	// Agents lists the names of agents that were active at compaction time.
	Agents []string `json:"agents,omitempty"`
}

// parseSkillState parses the serialized skill state from a PreservedContext.
// Returns nil (with no error) when the state string is empty.
func parseSkillState(state string) (map[string][]string, error) {
	if state == "" {
		return nil, nil
	}
	var raw skillStatePayload
	if err := json.Unmarshal([]byte(state), &raw); err != nil {
		return nil, fmt.Errorf("parse skill state: %w", err)
	}
	// Convert to a flat map for the AgentConfigRestorer interface.
	result := make(map[string][]string)
	if len(raw.Skills) > 0 {
		result["skills"] = raw.Skills
	}
	if len(raw.ToolsDelta) > 0 {
		result["tools_delta"] = raw.ToolsDelta
	}
	if len(raw.Agents) > 0 {
