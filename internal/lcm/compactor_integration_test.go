...
func TestIntegration_Compact_WithRealLLM(t *testing.T) {
	...
	tokensBefore, err := inner.store.GetContextTokenCount(ctx, sessionID)
	require.NoError(t, err)
	require.Greater(t, tokensBefore, int64(0))

	// Force compaction with real LLM via Compact.
	require.NoError(t, mgr.Compact(ctx, sessionID))

	tokensAfter, err := inner.store.GetContextTokenCount(ctx, sessionID)
	require.NoError(t, err)
	require.Less(t, tokensAfter, tokensBefore,
	...
	t.Logf("Compaction result: tokens %d -> %d", tokensBefore, tokensAfter)
	t.Logf("Summary:\n%s", summary.Content)
}
