...
SELECT summary_id, session_id, kind, content, token_count, file_ids, metadata, created_at, block_id, original_content FROM lcm_summaries WHERE summary_id = ?
`
...
SELECT summary_id, session_id, kind, content, token_count, file_ids, metadata, created_at, block_id, original_content FROM lcm_summaries WHERE session_id = ? ORDER BY created_at ASC
`
...
const listRecentSessionReadFiles = `-- name: ListRecentSessionReadFiles :many
SELECT session_id, path, read_at FROM read_files
WHERE session_id = ? AND read_at >= ?
ORDER BY read_at DESC
`

type ListRecentSessionReadFilesParams struct {
	SessionID string `json:"session_id"`
	ReadAt    int64  `json:"read_at"`
}

func (q *Queries) ListRecentSessionReadFiles(ctx context.Context, arg ListRecentSessionReadFilesParams) ([]ReadFile, error) {
	rows, err := q.query(ctx, q.listRecentSessionReadFilesStmt, listRecentSessionReadFiles, arg.SessionID, arg.ReadAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
