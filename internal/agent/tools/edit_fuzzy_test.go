...
// mockFiletrackerService implements filetracker.Service for tests.
...
func (m *mockFiletrackerService) LastReadTime(_ context.Context, _, path string) time.Time {
	return m.reads[path]
}
...
func newTestEditContext(t *testing.T) (editContext, string) {
	...
	return editContext{ctx, permissions, files, ft, tmpDir}, tmpDir
}
...
func TestEditTool_ReplaceContent_FuzzyIndentation(t *testing.T) {
	...
	edit.filetracker.RecordRead(edit.ctx, "test-session", testFile)
