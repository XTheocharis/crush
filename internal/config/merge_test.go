...
func TestConfigMerging(t *testing.T) {
	...
	t.Run("lsp_match_patterns_merged_and_deduplicated", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			LSP: LSPs{
				"test": {
					MatchPatterns: []string{"*.proto", "*.graphql"},
				},
			},
		}, Config{
			LSP: LSPs{
				"test": {
					MatchPatterns: []string{"*.graphql", "**/generated/**"},
				},
			},
		})

		require.NotNil(t, c)
		require.Equal(t, []string{"**/generated/**", "*.graphql", "*.proto"}, c.LSP["test"].MatchPatterns)
	})
	...
	t.Run("lcm_operational_memory_enabled_or_merged", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{
				LCM: &LCMOptions{
	...
	t.Run("validation_defaults_to_disabled", func(t *testing.T) {
		...
		if c.Options.Validation != nil {
			require.False(t, c.Options.Validation.Enabled)
		}
	})

	t.Run("validation_enabled_when_explicitly_set", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{
				Validation: &ValidationOptions{Enabled: true, AutoFix: true},
				TUI:        &TUIOptions{},
			},
		})
		require.NotNil(t, c)
		require.True(t, c.Options.Validation.Enabled)
		require.True(t, c.Options.Validation.AutoFix)
	})

	t.Run("validation_enabled_ored_on_merge", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{
				Validation: &ValidationOptions{Enabled: false, AutoFix: false},
				TUI:        &TUIOptions{},
			},
		}, Config{
			Options: &Options{
				Validation: &ValidationOptions{Enabled: true},
				TUI:        &TUIOptions{},
			},
		})
		require.NotNil(t, c)
		require.True(t, c.Options.Validation.Enabled)
		require.False(t, c.Options.Validation.AutoFix)
	})
	...
	t.Run("architect_model_overlay_replaces", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{
				ArchitectModel: &SelectedModel{
					Model:    "base-arch",
					Provider: "base-provider",
				},
				EditorModel: &SelectedModel{
					Model:    "base-edit",
					Provider: "base-provider",
				},
				TUI: &TUIOptions{},
		...
		require.NotNil(t, c)
		require.Equal(t, "overlay-arch", c.Options.ArchitectModel.Model)
		require.Equal(t, "overlay-provider", c.Options.ArchitectModel.Provider)
		require.Equal(t, "base-edit", c.Options.EditorModel.Model,
			"overlay without EditorModel should preserve base EditorModel")
	})

	t.Run("architect_editor_model_nil_overlay_preserves_base", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{
				ArchitectModel: &SelectedModel{
					Model:    "base-arch",
		...
		require.NotNil(t, c)
		require.Equal(t, "base-arch", c.Options.ArchitectModel.Model)
		require.Equal(t, "base-edit", c.Options.EditorModel.Model)
	})

	t.Run("architect_approval_required_ored_on_merge", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{
				Architect: &ArchitectOptions{ApprovalRequired: false},
				TUI:       &TUIOptions{},
			},
		}, Config{
			Options: &Options{
				Architect: &ArchitectOptions{ApprovalRequired: true},
				TUI:       &TUIOptions{},
			},
		})

		require.NotNil(t, c)
		require.True(t, c.Options.Architect.ApprovalRequired)
	})

	t.Run("architect_nil_in_first_config", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{TUI: &TUIOptions{}},
		}, Config{
			Options: &Options{
		...
		require.NotNil(t, c)
		require.NotNil(t, c.Options.Architect)
		require.True(t, c.Options.Architect.ApprovalRequired)
	})

	t.Run("architect_nil_in_second_config", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{
				Architect: &ArchitectOptions{ApprovalRequired: true},
				TUI:       &TUIOptions{},
	...
	t.Run("architect_defaults_to_not_required", func(t *testing.T) {
		c := exerciseMerge(t, Config{
			Options: &Options{TUI: &TUIOptions{}},
		}, Config{
			Options: &Options{TUI: &TUIOptions{}},
		...
		require.NotNil(t, c)
		if c.Options.Architect != nil {
			require.False(t, c.Options.Architect.ApprovalRequired)
		}
	})
}
