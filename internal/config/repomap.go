...
func (o RepoMapOptions) merge(t RepoMapOptions) RepoMapOptions {
	...
	o.ExcludeGlobs = append(o.ExcludeGlobs, t.ExcludeGlobs...)
...
// DefaultRepoMapMaxTokensLCM computes a higher budget when LCM manages
// context dynamically. The higher cap is safe because LCM controls
// conversation size and can reclaim space as needed.
func DefaultRepoMapMaxTokensLCM(modelContextWindow int) int {
	return min(max(modelContextWindow/8, 1024), 8192)
}
