...
> **Note**: The `pressure` and `target_tokens` parameters are passed through
> to the compaction manager. `pressure` overrides the compaction pressure tier
> (`compact_tool.go:59-64` → `manager.go:1025-1029`), and `target_tokens`
> overrides the soft token threshold (`manager.go:1048-1050`).
